...
public class AlarmsDao extends AbstractDao<Alarms, Long> {
    ...
    */
    public static class Properties {
        ...
        public final static Property Messageid = new Property(3, Long.class, "messageid", false, "MESSAGEID");
    ...
    @Override
    protected void attachEntity(Alarms entity) {
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getMessagesDao().getAllColumns());
            builder.append(" FROM ALARMS T");
            builder.append(" LEFT JOIN MESSAGES T0 ON T.'MESSAGEID'=T0.'_id'");
            builder.append(' ');
    ...
    protected Alarms loadCurrentDeep(Cursor cursor, boolean lock) {
        Alarms entity = loadCurrent(cursor, 0, lock);
        int offset = getAllColumns().length;

        Messages messages = loadCurrentOther(daoSession.getMessagesDao(), cursor, offset);
        entity.setMessages(messages);

        return entity;    
    }

    public Alarms loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<Alarms> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<Alarms> list = new ArrayList<Alarms>(count);
    ...
    protected List<Alarms> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<Alarms> queryDeep(String where, String... selectionArg) {
